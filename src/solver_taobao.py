...
from model import TaobaoDXY, TaobaoMLP
...
class Solver:
    ...
    def __init__(self,
        ...
        if self.method == "gdfm":
            ...
            self.p_d_xy = TaobaoDXY(y_size=self.y_class_num, d_size=self.d_size*self.d_nt).to(self.device)
            self.optimizer_dxy = get_optimizer(
                params["optimizer"], params)(self.p_d_xy.parameters())
            assert self.d_type in ["category"]
            
        self.p_y_x = TaobaoMLP(output_size=self.y_class_num).to(self.device)
        self.reg_p_y_x = TaobaoMLP(output_size=self.y_class_num).to(self.device)
    ...
    def update_step(self, x, y, d, y_mask, d_mask, streaming):
        ...
        if not streaming:
            ...
            _d_pred = d_pred.view((batch_size*self.d_nt*3, -1))
            _d_label = d.view((batch_size*self.d_nt*3,))
        ...
        # stream
            ...
            pred_d = self.p_d_xy(
                ...
                (batch_size, self.y_class_num, self.d_nt, 3, 2))
            
            # shape= batch_size, y_class_num, nt, 3, 2
            
            _sample_d_mask = F.one_hot(sample_d, 2).float()
            # _sample_d_mask.shape= batch_size, y_class_num, nt, 3, 2
            log_prob_d = torch.sum(F.log_softmax(pred_d, dim=4) * _sample_d_mask, dim=4).detach()
            # log_prob_d.shape= batch_size, y_class_num, nt, 3
            
            log_prob_y = F.log_softmax(y_logits, dim=1).unsqueeze(2).unsqueeze(3).repeat(1, 1, self.d_nt, 3)
            # shape= batch_size, y_class_num, nt, 3
            
            stream_d_loss = -torch.logsumexp(log_prob_d + log_prob_y, dim=1)
            # shape= batch_size, nt, 3
            
            stream_d_loss_mask = d_mask.float().view((batch_size, self.d_nt, 1)) # * (1 - y_mask.unsqueeze(1).float())
            stream_d_loss = stream_d_loss * stream_d_loss_mask / torch.sum(stream_d_loss_mask)
            stream_d_loss = stream_d_loss * self.weights.view((1, self.d_nt, 3))
            # shape= batch_size, nt
    ...
    def cal_weights(self, y, d):
        ...
        for i in range(self.d_nt):
            pdy = torch.zeros((3, 2, 2))
            for di in range(3):
                for vy in range(2):
                    for vd in range(2):
                        pdy[di, vd, vy] = torch.mean(torch.logical_and(y == vy, d[:, i, di] == vd).float()).item()
            ce = []
            for di in range(3):
                d_onehot = F.one_hot(d[:, i, di], 2).float()
                pd = torch.mean(d_onehot, dim=0).view((-1, 1))
                ce_y_d = torch.nansum(pdy[di, :, :] * torch.log(pd) - pdy[di, :, :] * torch.log(pdy[di, :, :]))
                ce.append(ce_y_d)
            ce = torch.stack(ce)
            self.conditional_entropy.append(ce)
        ...
        nt_weights = torch.exp(-self.beta * self.nt).view((-1, 1))
        ce_weights = torch.exp(-self.alpha * self.conditional_entropy)
        ce_weights = ce_weights / torch.max(ce_weights)
        nt_correction = 1.0 / torch.tensor(list(range(self.d_nt, 0, -1))).float().view((-1, 1))
    ...
    def pretrain(self):
        _local_path = "./pretrain_model/taobao_pretrain.pt"
        if os.path.isfile(_local_path):
            print("loading pretrain model, p_y_x")
            self.p_y_x.load_state_dict(torch.load(_local_path))
            self.reg_p_y_x.load_state_dict(torch.load(_local_path))
            self.reg_p_y_x.eval()
            self.p_y_x.train()
        else:
            ...
            torch.save(self.p_y_x.state_dict(), _local_path)
            self.reg_p_y_x.load_state_dict(self.p_y_x.state_dict())
            self.reg_p_y_x.eval()
            return
            
        if self.method in ["pretrain", "ce", "fnw", "oracle"]:
            return
        x, y, d, y_mask, d_mask, test_mask = self.pretrain_dataset["x"], self.pretrain_dataset["y"], self.pretrain_dataset["d"], \
            self.pretrain_dataset["y_mask"], self.pretrain_dataset["d_mask"], self.pretrain_dataset["test_mask"]
        x, y, d, y_mask, d_mask = get_train_data_from_batch(
